...
public abstract class BaseDictionary implements Cloneable, Closeable {
  ...
  //jdk1.8 to jdk1.6
  private static boolean SPACEBREAK_CODEPOINT_TEST(int cp){
  ...
  /*private static Predicate<Integer> SPACEBREAK_CODEPOINT = new Object(){}
...
;*/
  ...
  public List<String> readLineTokens(List<String> tokens) {
    List<String> lineTokens = new ArrayList<String>(tokens);
    lineTokens.add(EOS);
    return lineTokens;
  }
  ...
  public List<String> readLineTokens(String line) {
    ...
    int[] line_int_array = new int[line.length()];
    for(int i =0,j=line.length(); i<j; i++){
    	line_int_array[i]=line.codePointAt(i);
    }
    //for (int cp : line.codePoints().toArray()) {
    for (int cp : line_int_array) {
      if (SPACEBREAK_CODEPOINT_TEST(cp)) {
        if (token.length() > 0) {
  ...
  protected int getDictLine(List<String> tokens,
                            ...
                            List<Integer> labels) {
    List<Long> wordHashes = new ArrayList<Long>();
    lineWords.clear();
    labels.clear();
    int nTokens = 0;
    for (int i = 0; i < tokens.size(); i++) {
