...
/**
 * Base ResourceInput implementation that uses an array
 ...
 * <p>
 * From Lucene ByteBufferIndexInput.
 */
abstract class ByteBufferResourceInput extends ResourceInput implements RandomAccessInput {
  ...
  public static ByteBufferResourceInput newInstance(String resourceDescription, ByteBuffer[] buffers, long length, int chunkSizePower) {
    if (buffers.length == 1) {
      return new SingleBufferImpl(resourceDescription, buffers[0], length, chunkSizePower);
    } else {
      return new MultiBufferImpl(resourceDescription, buffers, 0, length, chunkSizePower);
    }
  }

  ByteBufferResourceInput(String resourceDescription, ByteBuffer[] buffers, long length, int chunkSizePower) {
  ...
  @Override
  public final ByteBufferResourceInput clone() {
    final ByteBufferResourceInput clone = buildSlice((String) null, 0L, this.length);
    try {
   ...
   */
  @Override
  public final ByteBufferResourceInput slice(String sliceDescription, long offset, long length) {
  ...
  protected ByteBufferResourceInput buildSlice(String sliceDescription, long offset, long length) {
    if (buffers == null) {
      throw new AlreadyClosedException("Already closed: " + this);
    }
    ...
    final ByteBufferResourceInput clone = newCloneInstance(getFullSliceDescription(sliceDescription), newBuffers, ofs, length);
    clone.isClone = true;
  ...
  protected ByteBufferResourceInput newCloneInstance(String newResourceDescription, ByteBuffer[] newBuffers, int offset, long length) {
    if (newBuffers.length == 1) {
      newBuffers[0].position(offset);
      return new SingleBufferImpl(newResourceDescription, newBuffers[0].slice(), length, chunkSizePower);
    } else {
      return new MultiBufferImpl(newResourceDescription, newBuffers, offset, length, chunkSizePower);
    }
  }
  ...
  static final class SingleBufferImpl extends ByteBufferResourceInput {

    SingleBufferImpl(String resourceDescription, ByteBuffer buffer, long length, int chunkSizePower) {
      super(resourceDescription, new ByteBuffer[] { buffer }, length, chunkSizePower);
      this.curBufIndex = 0;
      this.curBuf = buffer;
      buffer.position(0);
    }
    ...
    public void seek(long pos) throws IOException {
      try {
        ...
        if (pos < 0) {
          throw new IllegalArgumentException("Seeking to negative position: " + pos + " " + this, e);
        } else {
          throw new EOFException("seek past EOF: " + this);
        }
      } catch (NullPointerException npe) {
  ...
  static final class MultiBufferImpl extends ByteBufferResourceInput {
    private final int offset;

    MultiBufferImpl(String resourceDescription, ByteBuffer[] buffers, int offset, long length, int chunkSizePower) {
      super(resourceDescription, buffers, length, chunkSizePower);
      this.offset = offset;
      try {
        seek(0L);
      } catch (IOException ioe) {
    ...
    @Override
    protected ByteBufferResourceInput buildSlice(String sliceDescription, long ofs, long length) {
