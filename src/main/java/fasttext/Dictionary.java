...
import jdk.compatible.java.nio.charset.StandardCharsets;
...
public class Dictionary extends BaseDictionary {
  ...
  private Dictionary(Args args,
    ...
    this.words = words;
    this.word2int = new HashMap<Long, Integer>(word2intSize);
  ...
  protected int hashToId(long h) {
	  int wordint = word2int.containsKey(h)&&word2int.get(h)!=null? word2int.get(h): -1 ;
	  return wordint;
  }
  ...
  protected int getPruning(int id) {
    //return pruneIdx.getOrDefault(id, -1);
	  return pruneIdx.containsKey(id)? pruneIdx.get(id): word2int.get(-1) ;
  }
  ...
  public static Dictionary load(Args args, InputStreamFastTextInput is) throws IOException {
    ...
    Map<Integer, Integer> pruneIdx = new HashMap<Integer, Integer>(Math.max(0, pruneIdxSize));
    if (pruneIdxSize > 0) {
  ...
  public void saveToMMap(OutputStream os) throws IOException {
    List<Pair<Long, Integer>> orderedWord2int = new ArrayList<Pair<Long, Integer>>(size);
    for (Map.Entry<Long, Integer> w2i : word2int.entrySet()) {
      orderedWord2int.add(new Pair<Long, Integer>(w2i.getKey(), w2i.getValue()));
    }
    Comparator<Pair<Long, Integer>> comparator = new Comparator<Pair<Long, Integer>>(){
    	@Override
    	public int compare(Pair<Long, Integer> p1, Pair<Long, Integer> p2){
    		return (int)((long)p1.first()-(long)p2.first());
    	}
    	
    };
    Collections.sort(orderedWord2int, comparator);
    //orderedWord2int.sort(Comparator.comparing(Pair::first));

    List<Pair<Integer, Integer>> orderedPruneIdx = new ArrayList<Pair<Integer, Integer>>(Math.max(0, pruneIdxSize));
    if (pruneIdxSize > 0) {
      for (Map.Entry<Integer, Integer> p : pruneIdx.entrySet()) {
        orderedPruneIdx.add(new Pair<Integer, Integer>(p.getKey(), p.getValue()));
      }
    }
    Collections.sort(orderedWord2int, comparator);
    ...
    OutputStreamResourceOutput fos = null;
    try {
    	fos = new OutputStreamResourceOutput("dictionary", os, bufferSize);
       // dictionary utilities
       fos.writeInt(wordByteArrayLength);
       fos.writeInt(subwordsByteArrayLength);
       // dictionary meta data
