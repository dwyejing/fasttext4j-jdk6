...
public class Model {
  ...
  public void findKBest(int k,
    ...
    for (int i = 0; i < osz; i++) {
      ...
      heap.add(new Pair<Float, Integer>(stdLog(output.data[i]), i));
    }
    while (heap.size() > k) {
      heap.pollLast();
    }
  ...
  public void initTableNegative(long[] counts) {
    ...
    Integer[] negs_Integer_array = negs.toArray(new Integer[0]);
    for(int i=0,j=negs_Integer_array.length; i<j; i++){
    	negatives[i] = negs_Integer_array[i];
    }
