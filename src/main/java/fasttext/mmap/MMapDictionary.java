...
public class MMapDictionary extends BaseDictionary {
  ...
  private int entryByteArrayLength() {
    return wordByteArrayLength + subwordsByteArrayLength + 4 + 1 + 8 + 4;
  }
   ...
   * Consists in: 4 word length + word length
   */
  private int typeOffset() {
    return 4 + wordByteArrayLength;
  }

  /**
   * Offset to access to the count value in the entry byte array
   * Consists in: 4 word length + word length + 1 type encoding
   */
  private int countOffset() {
    return 4 + 8 + wordByteArrayLength;
  }

  /**
   * Offset to access to the subwords array in the entry byte array
   * Consists in: 4 word length + word length + 1 type encoding + 8 count
   */
  private int subwordsOffset() {
    return 4 + wordByteArrayLength + 1 + 8;
  }
