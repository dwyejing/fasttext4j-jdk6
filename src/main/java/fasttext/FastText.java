...
public class FastText {
  ...
  public List<FastTextPrediction> predict(String s, int k, float threshold) {
    ...
    List<FastTextPrediction> predictions = new ArrayList<FastTextPrediction>(dict.nLabels());
  ...
  public List<Vector> getWordVectors(List<String> words) {
    List<Vector> vecs = new ArrayList<Vector>(words.size());
  ...
  public List<Vector> getSentenceVectors(List<List<String>> sentences) {
    List<Vector> svecs = new ArrayList<Vector>(sentences.size());
  ...
  public List<Vector> textVectors(List<String> texts) {
    List<Vector> vecs = new ArrayList<Vector>(texts.size());
  ...
  private List<FastTextSynonym> findNN(Vector queryVec, int k, Set<String> banSet) {
    ...
    for (int i = 0; i < dict.nWords(); i++) {
      ...
      heap.add(new Pair<Float, String>(dp / queryNorm, word));
    }
    List<FastTextSynonym> syns = new ArrayList<FastTextSynonym>();
    int i = 0;
  ...
  public static FastText loadModel(String filename) throws IOException {
    ...
    if (f.isDirectory()) { 
    	// do not support this method  YJ_20180820
  ...
  private static FastText loadModel(InputStream in,
	  ...
	  InputStreamFastTextInput is = null;
    try {
    	is = new InputStreamFastTextInput(in);
  ...
  public void saveModel(OutputStream out) throws IOException {
    ...
    OutputStreamFastTextOutput os = null;
    try {
    	os= new OutputStreamFastTextOutput(out);
  ...
  public void saveAsMemoryMappedModel(String dirName) throws IOException {
    ...
    OutputStreamFastTextOutput os = null;
    try {
    	os = new OutputStreamFastTextOutput(new FileOutputStream(modelFile));
      signModel(FASTTEXT_FILEFORMAT_MAGIC_INT, version, os);
      args.save(os);
      os.writeBoolean(quant);
      os.writeBoolean(args.getQOut());
    ...
    try {
    	fos = new FileOutputStream(dictFile);
      dict.saveToMMap(fos);
    }finally{
    	fos.flush();
    	fos.close();
    ...
    FileOutputStream fos_sec = null;
    try  {
    	fos_sec = new FileOutputStream(inputFile);
      if (quant) {
        qinput.saveToMMap(fos_sec);
      } else {
        input.saveToMMap(fos_sec);
      }
    }finally{
    	fos_sec.flush();
    	fos_sec.close();
  ...
  /*public static void main(String[] args) throws Exception {
	...
	String inputModelPath = "E:\\Project\\workspace\\JFastText\\lid.bin";
  ...
  static public void main(String[] args) {
	    String path = "C:/Users/Administrator/Desktop/text/model_cooking.bin";
	    FastText model = null;
	    ...
	    try {
	      ...
	      List<FastTextPrediction> list = model.predict("Apple banana pear fruit watermelon orange", 5);
	      for(FastTextPrediction item : list){
	    	  System.out.println(item.label());
	      }

	      System.out.println("Nearest neighbors of \"apple\" are: ");
	      for (FastTextSynonym s : nearestNeighbors) {
	        System.out.println(s.word() + " -> " + s.cosineSimilarity());
	      }

	      // Analogy queries
	      List<FastTextSynonym> analogies = model.analogies("bala", "germany", "france", 10);
